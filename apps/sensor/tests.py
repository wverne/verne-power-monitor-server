...
class DeleteSensorTestCase(TestCase):
    ...
    def test_delete_with_logs(self):
        """
        Ensure that when a Sensor with PowerLogs is deleted, its PowerLogs are deleted as well.
        """
        ...
        response = self.client.post('/sensors/delete/{}/'.format(sensor_to_delete.id))

        # Response should be a redirect to the sensors list
        self.assertEqual(302, response.status_code)

        # Ensure that the chosen sensor and its logs were deleted
        self.assertFalse(Sensor.objects.filter(id=sensor_to_delete.id).exists())
        self.assertFalse(PowerLog.objects.filter(sensor_id=sensor_to_delete.id).exists())

        # Ensure that the other sensor and its logs were not affected
        self.assertTrue(Sensor.objects.filter(id=sensor_to_leave_in_place.id).exists())
        self.assertEqual(2, PowerLog.objects.filter(sensor_id=sensor_to_leave_in_place.id).count())
